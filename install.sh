...
sudo -u hardal bash -c 'cat packages.txt | sudo pacman -S --noconfirm --needed -' 

echo -e '\033[0;31mConfiguring system...\033[0m'
bash -c 'sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)"'
bash -c 'git clone -b main --depth=1 https://github.com/uiriansan/SilentSDDM && cd SilentSDDM && ./install.sh && cd .. && rm -rf SilentSDDM'
bash -c 'sudo sed -i "15c\\background = rei.mp4" /usr/share/sddm/themes/silent/configs/default.conf'
bash -c 'wget https://github.com/zen-browser/desktop/releases/latest/download/zen.linux-x86_64.tar.xz'
bash -c 'tar -xvf zen.linux-x86-64.tar.xz && rm zen.linux-x86_64.tar.xz && mv zen ~/zen'
bash -c 'cp kat.sh ~/kat.sh && cp zshrc ../.zshrc && cp zshenv ../.zshenv'
bash -c 'chmod +x ./lf/cleaner && chmod +x previewer'
